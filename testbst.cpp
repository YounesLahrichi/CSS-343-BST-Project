...
void test04() {
  ...
  cout << "* Testing constructor w/ vector, empty, size, height, contains, "
          "and count"
       << endl;
  BSTMap a;
  assert(a.empty());
  assert(a.height() == 0);
  vector<BSTMap::value_type> v{{"1", 1}, {"3", 3}, {"2", 2},
                               {"6", 6}, {"5", 5}, {"4", 4}};
  BSTMap b(v);
  assert(!b.empty());
  assert(b.size() == 6);
  assert(b.height() == 3);
  for (int i = 1; i < 7; i++) {
    assert(b.contains(to_string(i)));
    assert(b.count(to_string(i)) == 1);
  }
  BSTMap c(b);
  assert(c == b);
  assert(c != a);
